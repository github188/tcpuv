...
bool TcpClient::init()
{
	...
	ret = uv_tcp_init(&loop_, &client_context_->tcp_handle);
	if (ret) {
	...
	client_context_->tcp_handle.data = client_context_;
...
int TcpClient::sendToServer()
{
    ...
    while (!write_request_list_.empty()) {
        ...
        uv_write((uv_write_t*)&writereq->req, (uv_stream_t*)&client_context_->tcp_handle, &writereq->buf, 1, onWriteCallback);
...
bool TcpClient::connect(const char* ip, int port)
{
	...
	int r = uv_tcp_connect(&connect_handle_, &client_context_->tcp_handle, (struct sockaddr*) &bindAddr, onConnectCallback);
	if (r) {
...
bool TcpClient::startReconnect(void)
{
    ...
    client_context_->tcp_handle.data = this;
    return true;
}
...
{
    if (!uv_is_closing((uv_handle_t*)&client_context_->tcp_handle)) {
        uv_close((uv_handle_t*)&client_context_->tcp_handle, NULL);
    }
    startReconnect();
    uv_timer_stop(&reconnect_timer_);
...
void TcpClient::onClientCloseCallback(uv_handle_t* handle)
{
    ...
    if (handle == (uv_handle_t*)&parent->client_context_->tcp_handle && parent->is_reconnecting_) {
        int iret = 0;
...
void TcpClient::reconnectTimer(uv_timer_t* handle)
{
    ...
    if (theclass == NULL || !theclass->is_reconnecting_) {
        return;
    }
    AC_INFO("start reconnect to server:%s, port:%d ", theclass->server_ip_.c_str(), theclass->server_port_);
   
    int iret = uv_tcp_init(&theclass->loop_, &theclass->client_context_->tcp_handle);
    if (iret) {
        return;   //出错跳出循环
    }
    theclass->client_context_->tcp_handle.data = theclass->client_context_;
    ...
    if (iret) {        
        uv_close((uv_handle_t*)&theclass->client_context_->tcp_handle, NULL);
        return;
    }
    pAddr = (struct sockaddr*)&bind_addr;
        
    iret = uv_tcp_connect(&theclass->connect_handle_, &theclass->client_context_->tcp_handle, (const sockaddr*)pAddr, onConnectCallback);
    if (iret) {
        uv_close((uv_handle_t*)&theclass->client_context_->tcp_handle, NULL);
        return;
    }
